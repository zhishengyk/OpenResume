...
class MatchingService:
    def filter_and_score(
        ...
        must_have = [keyword.lower() for keyword in requested_keywords or profile.must_have_keywords]
        ...
        for draft in drafts:
            ...
            if salary_floor and draft.salary_min and draft.salary_min < salary_floor:
                continue
            if not any(target in draft.title.lower() or target in draft.jd_text.lower() for target in targets):
                continue
            ...
            role_component = 25.0 if any(target in draft.title.lower() for target in targets) else 12.0
            ...
            score = skill_component + role_component + level_component + domain_component + salary_component + location_component
            ...
            if "3-5年" in draft.experience_text and profile.years_experience < 3:
                risk_flags.append("experience gap")
