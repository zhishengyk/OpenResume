...
VARIANT_CONFIGS: dict[str, TaotianVariantConfig] = {
    "experienced": TaotianVariantConfig(
        variant="experienced",
        entry_url="https://talent.taotian.com/off-campus/position-list?lang=zh",
        channel="cdc_group_official_site",
        category_type=None,
        detail_path_template=(
            "https://talent.taotian.com/off-campus/position-detail?positionId={job_id}"
        ),
    ),
    "campus": TaotianVariantConfig(
        variant="campus",
        entry_url=(
            "https://talent.taotian.com/campus/position-list?campusType=freshman&lang=zh"
        ),
        channel="cdc_campus_group_official_site",
    ...
    "internship": TaotianVariantConfig(
        variant="internship",
        entry_url=(
...
class TaotianCareerClient:
    ...
    def _extract_token(self, html: str) -> str:
        ...
        if token:
            return token
        raise RuntimeError("Failed to extract Taotian career token")
