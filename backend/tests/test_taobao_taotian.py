...
from openresume_api.career_collectors.providers.taobao_taotian import (
    TaotianCareerClient,
    VARIANT_CONFIGS,
)
...
def test_variant_configs_map_to_expected_channels():
    assert VARIANT_CONFIGS["experienced"].channel == "cdc_group_official_site"
    assert VARIANT_CONFIGS["campus"].channel == "cdc_campus_group_official_site"
...
def test_detail_url_prefers_position_url_and_handles_relative_paths():
    ...
    assert (
        client.detail_url(
            variant="experienced",
            job_id="1",
            position_url="https://talent.taotian.com/custom/position/1",
        )
        == "https://talent.taotian.com/custom/position/1"
    )
    assert (
        client.detail_url(
            variant="experienced",
    ...
    assert (
        ...
        == "https://talent.taotian.com/campus/position-detail?positionId=2&campusType=internship"
    )
...
def test_search_payload_includes_category_type_for_campus():
    ...
    assert payload["channel"] == "cdc_campus_group_official_site"
